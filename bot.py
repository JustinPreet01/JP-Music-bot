...
@app.route('/')
def home():
    return "JP Music Bot is Online 24/7 with Cookies Bypass!"
...
# 🛡️ ULTRA-BYPASS YTDL OPTIONS (Cookies Integrated)
YTDL_OPTIONS = {
    ...
    'ignoreerrors': True,
    'cookiefile': 'cookies.txt',  # <--- YEH LINE MAGIC HAI!
    'extractor_args': {
        'youtube': {
...
async def on_ready():
    ...
    await bot.tree.sync()
    print(f"✅ {bot.user.name} is Online with Anti-Block!")
...
async def play(interaction: discord.Interaction, search: str):
    ...
    try:
        # Fresh extraction for each search
        ...
        print(f"Error: {e}")
        await interaction.followup.send("❌ Something went wrong. Make sure cookies.txt is valid and try again!")
